...
export default function AdminContainerList() {
    ...
    const currentItems = sortedCarbonList.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    return (
        <div className={styles.dashboard_container}>
                        ...
                        {currentItems.map((carbon) => (
                            <tr key={carbon.productId} className={styles.table_row}>
                                                ...
                                                src={
                                                    carbon.productImg
                                                        ? carbon.productImg.includes("/uploads/")
                                                            ...
                                                            : `${backHost}/uploads/container/${carbon.productImg}`
                                ...
                                <td className={styles.col_action}>
                                    <button className={styles.edit_btn} onClick={() => navigate(`/mypage/admin/containers/detail/${carbon.productId}`, { state: { carbonData: carbon } })}>
                                        <EditIcon className={styles.edit_icon} />
                                    </button>
                                    <button className={styles.delete_btn} onClick={() => handleDelete(carbon.productId)}>
            ...
            <div className={styles.pagination}>
                <button className={styles.page_btn_nav} onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1}>
                    <ChevronLeftIcon fontSize="small" /> 이전
                </button>
                <div className={styles.page_numbers}>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                        <button key={page} className={`${styles.page_num} ${currentPage === page ? styles.active : ''}`} onClick={() => setCurrentPage(page)}>
                            {page}
                        </button>
                    ))}
                </div>
                <button className={styles.page_btn_nav} onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0}>
