...
const UserDelAccount = () => {
  ...
  const handleDeleteAccount = async () => {
    ...
    //서버로 탈퇴 요청 보내기(비밀번호 검증 및 DB 상태값 변경)
    try {
      const token = localStorage.getItem("accessToken");
      ...
      Swal.fire({
        ...
        allowOutsideClick: false, // 👈 알람창 바깥 클릭해도 안 닫히게 설정
      }).then((result) => {
        // 3. 사용자가 '확인' 버튼을 확실히 눌렀을 때만 로그아웃 실행!
        if (result.isConfirmed) {
          logout();
          // 만약 logout() 안에 navigate("/")가 없다면 여기서 navigate("/") 추가
        }
      });
      ...
      <div className={styles.main_container}>
        <section>
          <div className={styles.main_text}>
            ...
            <h2>탈퇴를 진행하시겠습니까?</h2>
            <p>
              회원님은 <span>2023년 3월</span>부터 저희와 함께했어요
            </p>
          </div>
        </section>
        <section>
